...
# ---------------------
sed -i -e "s|###ORACLE_BASE###|$ORACLE_BASE|g" $ORACLE_BASE/$GSM_RSP && \
sed -i -e "s|###GSM_HOME###|$GSM_HOME|g" $ORACLE_BASE/$GSM_RSP

# Install Oracle binaries
cd $ORACLE_BASE         && \
unzip $GSM_INSTALL_FILE && \
rm $GSM_INSTALL_FILE    && \
$ORACLE_BASE/gsm/runInstaller -silent -showProgress -ignorePrereqFailure -ignoreSysPrereqs -waitforcompletion -responseFile $ORACLE_BASE/$GSM_RSP 2>/dev/null || exit 0 && \
cd $HOME
