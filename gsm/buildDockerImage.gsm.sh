...
Usage: buildDockerImage.gsm.sh -v [version] [-e | -s | -x] [-i] [-o] [Docker build option]
...
while getopts "hesxiv:o:" optname; do
  case "$optname" in
    ...
    *)
    # Should not occur
      echo "Unknown error while processing options inside buildDockerImage.gsm.sh"
      ;;
  esac
done
...
IMAGE_NAME="oracle/database:$VERSION-$EDITION-gsm"
...
cat << EOF
  Oracle Database Docker Image for '$EDITION' version $VERSION with GSM is ready to be extended: 
